...
contract Auction {
    ...
    // constructor(uint auctionDurationDays, uint initialOffer) public {
    constructor() public {
        beneficiary = msg.sender;
        auctionEndTime = block.timestamp + (1 * 1 days);
    ...
    function getBalance(address account) external view returns (uint) {
        return beneficiary == highest.bidder ? 0 : balance[account];
    }
